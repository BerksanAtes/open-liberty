...
public final class WSSecurityUtil {
    ...
    public static Element getSecurityHeader(Document doc, String actor) throws WSSecurityException {
        ...
        Element soapHeaderElement = getDirectChildElement(
    ...
    public static List<Element> findElements(
                                             WSEncryptionPart part, CallbackLookup callbackLookup, Document doc) throws WSSecurityException {
...
//        }
    ...
    public static Element findWsseSecurityHeaderBlock(
                                                      Document doc,
                                                      Element envelope,
                                                      boolean doCreate) throws WSSecurityException {
    ...
    public static Element findWsseSecurityHeaderBlock(
                                                      ...
                                                      boolean doCreate) throws WSSecurityException {
        String soapNamespace = WSSecurityUtil.getSOAPNamespace(doc.getDocumentElement());
        Element header = getDirectChildElement(
                                               doc.getDocumentElement(),
                                               WSConstants.ELEM_HEADER,
                                               soapNamespace);
        if (header == null) { // no SOAP header at all
            ////System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() header is null" );
            if (doCreate) {
                ////System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() in doCreate");
                if (!System.getProperty("java.specification.version").startsWith("1.")) {
                    ////System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() in doCreate and it is java9+");
                    try {
                        Node node = null;
                        try {
                            Method method = doc.getClass().getMethod("getEnvelope");
                            node = (Node) method.invoke(doc);
                            ////System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() node = (Node)method.invoke(doc): " + node);
                        ...
                        if (node != null) {
                            ...
                            ////System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() header = createElementInSameNamespace(node, WSConstants.ELEM_HEADER): " + header);

                        } else {
                            header = createElementInSameNamespace(doc.getDocumentElement(), WSConstants.ELEM_HEADER);
                            //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() header = createElementInSameNamespace(doc.getDocumentElement(), WSConstants.ELEM_HEADER): " + header);
                        }
                        doc.importNode(header, true);
                        //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() doc.importNode(header, true): " + doc);
                        header = (Element) getDomElement(header);
                        //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() header: " + header);

                        header = prependChildElement(envelope, header);
                        //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() header with prependChildElement: " + header);                       
                    ...
                    //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() not in java9+" );                        
                    header = createElementInSameNamespace(envelope, WSConstants.ELEM_HEADER);
                    //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() createElementInSameNamespace(envelope, WSConstants.ELEM_HEADER) header = "  + header);  
                    header = prependChildElement(envelope, header);
                    //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() prependChildElement(envelope, header) header = "  + header);  

                }

            } else {
                //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() returning null");                   
                return null;
            }
        }

        String actorLocal = WSConstants.ATTR_ACTOR;
        //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() actorLocal: " + actorLocal);                   
        ...
        if (foundSecurityHeader != null) {
            //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() foundSecurityHeader != null: " + foundSecurityHeader);                      
            return foundSecurityHeader;
        } else if (doCreate) {
            //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() doCreate: ");                      
            ...
            //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() foundSecurityHeader: " + foundSecurityHeader); 
            doc.importNode(foundSecurityHeader, true);
            foundSecurityHeader = (Element) getDomElement(foundSecurityHeader);
            //System.out.println("WSSecurityUtil: findWsseSecurityHeaderBlock() foundSecurityHeader getDomElement: " + foundSecurityHeader); 
    ...
    public static int decodeAction(
                                   ...
                                   WSSConfig wssConfig) throws WSSecurityException {
    ...
    public static void checkAllElementsProtected(
                                                 ...
                                                 int action,
                                                 QName[] requiredParts) throws WSSecurityException {
        ...
        if (requiredParts != null) {
            for (int i = 0; i < requiredParts.length; i++) {
                ...
                boolean found = false;
                for (Iterator<WSSecurityEngineResult> iter = results.iterator(); iter.hasNext() && !found;) {
    ...
    public static void checkSignsAllElements(
                                             ...
                                             String[] requiredIDs) throws WSSecurityException {
        int resultAction = ((java.lang.Integer) resultItem.get(WSSecurityEngineResult.TAG_ACTION)).intValue();
        if (resultAction != WSConstants.SIGN) {
            throw new IllegalArgumentException("Not a SIGN result");
        }

        List<WSDataRef> signedElemsRefList = (List<WSDataRef>) resultItem.get(WSSecurityEngineResult.TAG_DATA_REF_URIS);
     ...
     */
    public static List<Node> listChildren(
                                          final Node parent) {
    ...
    public static List<Node> newNodes(
        ...
        for (final Iterator<Node> bpos = b.iterator(); bpos.hasNext();) {
            final Node bnode = bpos.next();
            final String bns = bnode.getNamespaceURI();
            final String bln = bnode.getLocalName();
            boolean found = false;
            for (final Iterator<Node> apos = a.iterator(); apos.hasNext() && !found;) {
                final Node anode = apos.next();
                final String ans = anode.getNamespaceURI();
                final String aln = anode.getLocalName();
                final boolean nsmatch = ans == null ? ((bns == null) ? true : false) : ((bns == null) ? false : ans.equals(bns));
                final boolean lnmatch = aln == null ? ((bln == null) ? true : false) : ((bln == null) ? false : aln.equals(bln));
                if (nsmatch && lnmatch) {
                    found = true;
                }
            }
    ...
    public static Node getDomElement(Node node) throws WSSecurityException {
        ...
        if (!System.getProperty("java.version").startsWith("1.")) {
            try {
                ...
                log.info("Not a saaj node with java9");
            } catch (Exception e) {
                throw new WSSecurityException(WSSecurityException.INVALID_SECURITY);
            }
        }
        //log.info("WSSecurityUtil: exiting getDomElement() node == " + node);
