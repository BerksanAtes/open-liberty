...
public class PhaseInterceptorChain implements InterceptorChain {
    ...
    public synchronized boolean doIntercept(@Sensitive Message message) { // Liberty change: @Sensitive is added to parameter
        updateIterator();
    ...
    // Liberty change: @Sensitive is added to first 2 parameters below
    private void doDefaultLogging(@Sensitive Message message, @Sensitive Exception ex, String description) {
        ...
        if (mode == FaultMode.CHECKED_APPLICATION_FAULT) {
            if (isFineLogging) {
                ...
                LogUtils.log(LOG, Level.INFO, // Liberty change: Level.INFO is replaced with Level.FINE
                             "Application " + description
    ...
    @SuppressWarnings("unchecked")
    public void unwind(@Sensitive Message message) {  // Liberty change: @Sensitive is added to the parameter
