...
public class UsernameTokenProcessor implements Processor {
    ...
    handleUsernameToken(
        ...
        if (wssConfig != null) {
            ...
            System.out.println("WSS4J_DEBUG: timeout read from config: " + utTTL);
        ...
        System.out.println("WSS4J_DEBUG: Testing replay attack for nonce: " + ut.getNonce() );
        
        // Test for replay attacks
        ReplayCache replayCache = data.getNonceReplayCache();
        if (replayCache != null && ut.getNonce() != null) {
            System.out.println("WSS4J_DEBUG: Found Nonce in UNT. ");
            if (replayCache.contains(ut.getNonce())) {
                System.out.println("WSS4J_DEBUG: Nonce found in cache. ");
                throw new WSSecurityException(
            ...
            if (created == null || utTTL <= 0) {
                System.out.println("WSS4J_DEBUG: Caching nonce for default time.");
                replayCache.add(ut.getNonce());
            } else {
                System.out.println("WSS4J_DEBUG: Caching nonce for " + (utTTL+1L));
