...
public class WSSecEncrypt extends WSSecEncryptedKey {
    ...
    public static List<String> doEncryption(
        ...
        for (int part = 0; part < references.size(); part++) {
            ...
            for (Element elementToEncrypt : elementsToEncrypt) {
				elementToEncrypt = (Element) WSSecurityUtil.getDomElement(elementToEncrypt);
                String id = 
            ...
            if (part != (references.size() - 1)) {
                try {
                    keyInfo = new KeyInfo((Element) WSSecurityUtil.cloneElement(doc, (Element) WSSecurityUtil.getDomElement(keyInfo.getElement())), null);
                } catch (Exception ex) {
                    throw new WSSecurityException(
    ...
    private static String encryptElement(
        ...
        try {
            ...
            if ("Header".equals(modifier)) {
				...
				elem = (Element) WSSecurityUtil.getDomElement(elem);
    ...
    private KeyInfo createKeyInfo() throws WSSecurityException {
		...
		keyInfoElement = (Element) WSSecurityUtil.getDomElement(keyInfoElement);
