...
public class DataWriterImpl<T> extends JAXBDataBase implements DataWriter<T> {
    ...
    boolean noEscape;
    private final JAXBDataBinding databinding; // @BA final is added
    ...
    public Marshaller createMarshaller(Object elValue, MessagePartInfo part) {
        ...
        try {
            //marshaller = context.createMarshaller(); Liberty change: removed line
            marshaller = databinding.getJAXBMarshaller(); //Liberty change: added line

            // Liberty change: if clause and comment above it is added below
            ...
            if (Boolean.FALSE.equals(marshaller.getProperty(Marshaller.JAXB_FRAGMENT))
             && marshaller.getAttachmentMarshaller() == null) {
                ...
                marshaller.setProperty(Marshaller.JAXB_FRAGMENT, Boolean.TRUE); // Liberty change: true is replaced by Boolean.TRUE
            ...
            } //Liberty change: close of added if clause
    ...
    public void write(Object obj, MessagePartInfo part, T output) {
        ...
        Marshaller marshaller = null; //Liberty change: added line
        if (obj != null
            ...
            if (obj instanceof Exception
                ...
                && Boolean.TRUE.equals(part.getProperty(JAXBDataBinding.class.getName()
                                                        + ".CUSTOM_EXCEPTION"))) {
                marshaller = createMarshaller(obj, part);             //Liberty change: added line
                JAXBEncoderDecoder.marshallException(marshaller,      // Liberty change: createMarshaller(obj, part) is replaced by marshaller
                                                     (Exception)obj,
                                                     part,
                                                     output);
                ...
                if (!honorJaxbAnnotation || anns.length == 0) {
                    marshaller = createMarshaller(obj, part);                     //Liberty change: added line
                    JAXBEncoderDecoder.marshall(marshaller, obj, part, output);   // Liberty change: createMarshaller(obj, part) is replaced by marshaller
                    onCompleteMarshalling();
            ...
            marshaller = createMarshaller(null, part); //Liberty change: added line
            JAXBEncoderDecoder.marshallNullElement(marshaller, output, part); // Liberty change: createMarshaller(null, part) is replaced by marshaller

            onCompleteMarshalling();
        }
        //Liberty change: added line below (need to check if onCompleteMarshalling() does the same thing as this line)
