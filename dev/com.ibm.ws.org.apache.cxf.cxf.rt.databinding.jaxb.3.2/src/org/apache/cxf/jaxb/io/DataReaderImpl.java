...
public class DataReaderImpl<T> extends JAXBDataBase implements DataReader<T> {
    ...
    private Unmarshaller createUnmarshaller() {
        ...
        try {
            // Liberty change: below code commented out is original CXF 3.3
            // Unmarshaller um = null;
            // um = context.createUnmarshaller();
            // if (databinding.getUnmarshallerListener() != null) {
            //     um.setListener(databinding.getUnmarshallerListener());
            // }
            // Liberty change: above code is original CXF 3.3
            um = databinding.getJAXBUnmarshaller();
             if (setEventHandler) {
    ...
    public Object read(MessagePartInfo part, T reader) {
        ...
        try {
            Object obj = JAXBEncoderDecoder.unmarshall(um, reader, part,
                                                 unwrapJAXBElement);
            // databinding.releaseJAXBUnmarshaller(unmarshaller); //Liberty change: not sure if JAXBUtils.closeUnmarshaller(um) provides the same functionality
            onCompleteUnmarshalling();
